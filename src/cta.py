...
def cta_retriever(candidates_list, correct=None, max_depth=0):
    ...
    while index < len(candidates_list) and current_depth <= max_depth:
        ...
        for candidates in candidates_list:
            ...
            if current_depth == 0:
                instance_dicts[index], amount_of_nonetypes, explore_next = add_to_instance_dict(
                    candidates, instance_dicts[index]
                )

                best_key, best_values[index], correct_key = find_best_key(
                ...
                instance_dicts[index], num_of_none, explore_next = add_to_instance_dict(
                    cands, instance_dicts[index], tree_exploration=True
                )
            ...
            if correct != None:
                ...
                if correct_key != best_key and correct_key != None:
                    ...
                    print(
                        f"return key: {best_key} confidence: {confidence} best-value/len(candidates): {best_values[index]/len(candidates)} percentage Nonetypes: {amount_of_nonetypes/len(candidates)}"
                    )
...
def add_to_instance_dict(candidates, instance_dict, tree_exploration=False):
    ...
    for candidate in candidates:
        ...
        for statement in candidate.statements:
            if (statement.property == 31 and tree_exploration == False) or (
                statement.property == 279 and tree_exploration == True
            ):
...
if __name__ == "__main__":
    ...
    candidates_objects = pickle_load(f"{PICKLE_FILE_NAME}", is_dump=False)
    ...
    print(
        f"max depth successes: {max_depth_successes}   max depth failures: {max_depth_failures}   success/failures: {max_depth_successes/max_depth_failures}"
    )
